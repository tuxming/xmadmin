...
import { UserAddIcon, DeleteIcon, EditIcon, ScanIcon, } from '../../components/icon/svg/Icons';
import { PermissionListComponent, PermissionEdit, PermissionDelete } from './index'
    ...
    return <>
        ...
        <Space wrap>
            <Tooltip title={t("新增权限")}>
                <Button type='primary' size={size} icon={<UserAddIcon type='primary'/>} onClick={onCreate}>{!onlyIcon && t('新增')}</Button>
            </Tooltip>
            ...
            <Tooltip title={t("删除权限")}>
                <Button type='primary' size={size} icon={<DeleteIcon type='ghostPrimary' danger/>} 
        ...
        <Divider />
        <PermissionListComponent onSelect={onTableSelectChange} query={query} refresh={refresh}/>
        <PermissionEdit open={isOpenEdit} onClose={onAddClose} permission={permission}/>
        <PermissionDelete permissions={deletes} successCall={onRefresh}/>
        <PermissionScan open={isOpenScan} onClose={onScanClose}></PermissionScan>
    </>
}