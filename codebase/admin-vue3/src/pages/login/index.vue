        ...
        <div class="login-bg-color">
            <div class="login-wrap">
                ...
                <div class="login-container" v-if="pageType === 'login'">
                    ...
                    <t-form ref="form" :data="formData" class="item-container" :rules="rules" @submit="onSubmit"
                        label-width="80px" style="padding: 0 20px">
                        <!-- Login Type Tabs (Account / Email / Phone) -->
                        <template v-if="loginType === 'account'">
                            <t-form-item :label="t('用户名')" name="username">
                                <t-input v-model="formData.username" size="large" :placeholder="t('请输入账号')"
                            ...
                            <t-form-item :label="t('密码')" name="password">
                                <t-input v-model="formData.password" size="large" type="password" clearable
                                    :placeholder="t('请输入密码')" autocomplete="current-password">
                        ...
                        <template v-else-if="loginType === 'mail'">
                            ...
                            <t-form-item :label="t('邮件验证码')" name="mailCode">
                                <t-input v-model="formData.mailCode" size="large" :placeholder="t('请输入邮件验证码')">
                                    <template #suffix>
                                        <t-button variant="text" size="small" :disabled="!canSendCode"
                                            @click="sendCode('mail')">
                        ...
                        <!-- Common Captcha -->
                        <t-form-item :label="t('验证码')" name="code"
                            v-if="loginType === 'account' || loginType === 'mail' || loginType === 'phone'">
                            <div style="display: flex; align-items: flex-start; width: 100%;">
                                <t-input v-model="formData.code" size="large" :placeholder="t('请输入验证码')"
                                    style="border-top-right-radius: 0; border-bottom-right-radius: 0; flex: 1; width: 0;">
                                </t-input>
                                <img :src="codeUrl"
                                    style="height: 40px; width: 120px; cursor: pointer; border-radius: 0 4px 4px 0; object-fit: cover; flex-shrink: 0;"
                                    @click="refreshCaptcha" />
                            </div>
                        </t-form-item>

                        <div style="padding: 5px 15px 15px 15px; text-align: center;">
                            <t-button block size="large" type="submit" style="width: 100%" :loading="loading"> {{
                                t('登录') }} </t-button>
                        </div>
                    </t-form>
                    ...
                    <div class="other-login">
                        ...
                        <t-tooltip :content="t('账号密码登录')">
                            <t-button shape="circle" variant="text" theme="primary" style="margin-left: 15px;"
                                @click="loginType = 'account'" :ghost="loginType !== 'account'">
                                <icon-font name="user" />
                            </t-button>
                        </t-tooltip>
                        <t-tooltip :content="t('邮件登录')">
                            <t-button shape="circle" variant="text" theme="primary"
                                style="margin-left: 15px; margin-right: 15px;" @click="loginType = 'mail'"
                                :ghost="loginType !== 'mail'">
                                <icon-font name="mail" />
                            </t-button>
                        </t-tooltip>
                        <t-tooltip :content="t('电话登录')">
                            <t-button shape="circle" variant="text" theme="primary" @click="loginType = 'phone'"
                                :ghost="loginType !== 'phone'">
                                <icon-font name="call" />
                            </t-button>
                        </t-tooltip>
                    </div>
                ...
                <div class="login-container" v-else>
                    ...
                    <div class="login-form-wrap" style="padding: 0 20px; min-height: 200px;">
                        ...
                        <t-form v-if="forgetStep === 0" ref="forgetFormRef" :data="forgetData" :rules="forgetRules"
                            @submit="onForgetNext" label-width="80px">
                            ...
                            <t-form-item :label="t('验证码')" name="captcha">
                                <div style="display: flex; align-items: flex-start; width: 100%;">
                                    <t-input v-model="forgetData.captcha" size="large" :placeholder="t('请输入验证码')"
                                        style="border-top-right-radius: 0; border-bottom-right-radius: 0; flex: 1; width: 0;">
                                    </t-input>
                                    <img :src="codeUrl"
                                        style="height: 40px; width: 120px; cursor: pointer; border-radius: 0 4px 4px 0; object-fit: cover; flex-shrink: 0;"
                            ...
                            <div style="padding: 5px 15px 15px 15px; text-align: center;">
                                <t-button block size="large" type="submit" style="width: 100%"> {{ t('下一步') }}
                                </t-button>
                            </div>
                        </t-form>

                        <!-- Step 2: Reset Password -->
                        <t-form v-else-if="forgetStep === 1" ref="resetFormRef" :data="resetData" :rules="resetRules"
                            @submit="onResetSubmit" label-width="80px">
                            <t-form-item :label="t('新密码')" name="password">
                                <t-input v-model="resetData.password" size="large" type="password" clearable
                                    :placeholder="t('请输入密码')">
                                    <template #prefix-icon><t-icon name="lock-on" /></template>
                                </t-input>
                            </t-form-item>
                            <t-form-item :label="t('确认密码')" name="repassword">
                                <t-input v-model="resetData.repassword" size="large" type="password" clearable
                            ...
                            <div
                                style="display: flex; justify-content: space-around; margin-bottom: 15px; padding-top: 15px;">
                                <t-button size="large" variant="outline" style="width: 120px" @click="forgetStep = 0">{{
                                    t('上一步')
                                    }}</t-button>
                                <t-button size="large" type="submit" style="width: 120px" :loading="loading">{{ t('提交')
                                    }}</t-button>
                            </div>
                        </t-form>
                        ...
                        <div v-else style="text-align: center; padding-top: 20px;">
                            <t-icon
                                :name="resetResult.status === 'success' ? 'check-circle-filled' : 'close-circle-filled'"
                                size="48px"
                                :style="{ color: resetResult.status === 'success' ? 'var(--td-success-color)' : 'var(--td-error-color)' }" />
                            <div style="margin-top: 16px; font-size: 16px;">{{ resetResult.status === 'success' ?
                                t('重置成功') :
                                t(resetResult.msg) }}</div>
