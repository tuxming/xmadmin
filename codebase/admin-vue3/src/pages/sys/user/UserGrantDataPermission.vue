    ...
    <div :style="wrapperStyle">
        <div style="position: relative; margin-bottom: 15px;">
            ...
            <t-tooltip :content="t('添加数据权限')">
                <t-button theme="primary" shape="circle" style="position: absolute; bottom: -5px; right: 30px;"
                    @click="onAdd">
        ...
        <div style="margin-bottom: 20px;">
            <TableComponent :page-size="20" :query="query" :api-url="`${api.user.dataPermissions}?type=1&id=${userId}`"
                :width="465" :height="250" :columns="userCols" :refresh="userRefresh" :on-select="onSelect"
                class="need-buttom-border" />
        </div>

        <div>
            <TableComponent :page-size="20" :query="query" :api-url="`${api.user.dataPermissions}?type=2&id=${userId}`"
                :width="565" :height="250" :columns="deptCols" :refresh="deptRefresh" :on-select="onSelect"
                class="need-buttom-border" />
        </div>
    </div>
</template>
