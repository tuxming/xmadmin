<template>
    <Modal :open="visible" :title="title" :width="1100" :show-mask="false" :offset="modalOffset"
                ...
                <t-tabs v-model="activeTab" placement="left" class="user-edit__tabs">
                    <t-tab-panel value="1" :label="t('基本信息')">
                        <div class="user-edit__panel">
                            <UserEditBasicInfo :user="editUser" :modal-pos="modalPos" vertical
                                @handle-change="onHandleChange" @update-user="doUpdate" />
                        </div>
                    </t-tab-panel>
                    <t-tab-panel value="2" :label="t('账户与安全')">
                        <div class="user-edit__panel">
                            <UserEditSecurity :user="editUser" :modal-pos="modalPos" vertical
                                @handle-change="onHandleChange" />
                        </div>
                    </t-tab-panel>
                    <t-tab-panel value="3" :label="t('数据权限')">
                        <div class="user-edit__panel">
                            <UserGrantDataPermission :user-id="user.id" :title-level="5"
                    ...
                    <t-tab-panel value="4" :label="t('角色')">
                        <div class="user-edit__panel">
                            <UserGrantRole :user-id="user.id" :title-level="5"
                                :title-style="{ marginTop: '5px', marginBottom: '20px' }"
                                :wrapper-style="{ paddingRight: '20px' }" />
                        </div>
                    </t-tab-panel>
