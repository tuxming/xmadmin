<template>
    <Modal :open="visible" :title="title" :width="500" :show-mask="false" @close="onModalClose(false)">
        <div style="width: 100%">
            <div style="padding: 0px 20px 10px 20px; margin: 0px auto">
                ...
                <t-form ref="formRef" :data="formData" layout="horizontal" :rules="rules" @submit="onFinish"
                    ...
                    <t-form-item name="groupName" :label="t('字典名')">
                        <t-auto-complete v-model="formData.groupName" :options="groupOptions"
                    ...
                    <t-form-item name="dictValue" :label="t('值')">
                        ...
                        <template v-else-if="dictType === 3">
                            <t-upload v-model="formData.dictValue" theme="image" accept="image/*"
                                :request-method="customUpload" :max="1" />
                        </template>
                        <template v-else>
