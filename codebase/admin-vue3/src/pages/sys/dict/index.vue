<template>
  <div style="height: 100%">
    <DoubleColumnLayout :width="containerWidth" :left-width="leftWidth">
      <template #left>
        <div :style="{ height: pos.height ? (pos.height - 85) + 'px' : '100%' }">
          <t-space>
            <AuthButton theme="primary" variant="outline" shape="circle" :tip="t('新增字典名')" :required-permissions="permission.dict.groupAdd.expression" @click="onCreateGroup">
              <template #icon><t-icon name="add" /></template>
            </AuthButton>
            <AuthButton theme="default" variant="outline" shape="circle" :tip="t('编辑字典名')" :required-permissions="permission.dict.groupUpdate.expression" @click="onEditGroup">
              <template #icon><t-icon name="edit" /></template>
            </AuthButton>
            <AuthButton theme="danger" variant="outline" shape="circle" :tip="t('删除字典名')" :required-permissions="permission.dict.groupDelete.expression" @click="onDeletGroup">
      ...
      <template #right>
        <div style="padding-left: 15px; height: 100%; display: flex; flex-direction: column;">
          ...
          <t-divider size="8px" />
          <t-space wrap>
            <AuthButton theme="primary" :tip="t('新增字典数据')" :required-permissions="permission.dict.add.expression" @click="onCreateDict">
              <template #icon><t-icon name="add" /></template>
              {{ !onlyIcon ? t('新增') : '' }}
            </AuthButton>
            <AuthButton theme="default" :tip="t('编辑字典数据')" :required-permissions="permission.dict.update.expression" @click="onEditDict">
              <template #icon><t-icon name="edit" /></template>
              {{ !onlyIcon ? t('编辑') : '' }}
            </AuthButton>
            <AuthButton theme="danger" :tip="t('删除字典数据')" :required-permissions="permission.dict.delete.expression" @click="onDeleteDict">
              <template #icon><t-icon name="delete" /></template>
              {{ !onlyIcon ? t('删除') : '' }}
            </AuthButton>
          </t-space>
    ...
    <DictGroupEdit
      v-if="isGroupEditOpen"
      v-model:open="isGroupEditOpen"
      :group="editGroupObj"
      :title="title"
