<template>
    <div class="permission-page">
        ...
        <t-space wrap>
            <AuthButton theme="primary" :tip="t('新增权限')" :required-permissions="permission.permission.create.expression"
                @click="onCreate">
                <template #icon><t-icon name="add" /></template>
                {{ !onlyIcon ? t('新增') : '' }}
            </AuthButton>

            <AuthButton theme="default" :tip="t('编辑权限')" :required-permissions="permission.permission.update.expression"
                @click="onEdit">
                <template #icon><t-icon name="edit" /></template>
                {{ !onlyIcon ? t('编辑') : '' }}
            </AuthButton>

            <AuthButton theme="default" :tip="t('扫描权限')" :required-permissions="permission.permission.scan.expression"
                @click="onScan">
                <template #icon><t-icon name="scan" /></template>
                {{ !onlyIcon ? t('扫描') : '' }}
            </AuthButton>

            <AuthButton theme="danger" :tip="t('删除权限')" :required-permissions="permission.permission.delete.expression"
                @click="onDelete">
        ...
        <div class="table-container">
            <PermissionList :query="query" :refresh="refresh" @select="onTableSelectChange" />
        </div>

        <PermissionEdit v-if="isOpenEdit" v-model:open="isOpenEdit" :permission="currPermission" :title="title"
            @close="onAddClose" />
    </div>
</template>
