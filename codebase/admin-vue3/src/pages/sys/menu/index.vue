        ...
        <DoubleColumnLayout :width="containerWidth" :left-width="300">
            <template #left>
                <div style="height: 100%; display: flex; flex-direction: column;">
                    <div style="display: flex; justify-content: flex-end; padding: 10px;">
                        <t-space>
                            <AuthButton theme="primary" variant="outline" shape="circle" :tip="t('添加菜单')"
                                :required-permissions="permission.menu.create.expression" @click="prepareAdd">
                                <template #icon><t-icon name="add" /></template>
                            </AuthButton>
                            <AuthButton theme="danger" variant="outline" shape="circle" :tip="t('删除菜单')"
                                :required-permissions="permission.menu.delete.expression" @click="doDelete">
                    ...
                    <div style="flex: 1; overflow: auto; padding: 0 10px;">
                        <t-tree v-if="refreshTree" :data="treeData" hover activable :expanded="expandedKeys"
                            :keys="{ label: 'title', value: 'key', children: 'children' }" style="text-align: left;"
                            :load="onLoadData" :line="true" lazy @active="onSelect" @expand="onExpand">
                            <template #label="{ node }">
                                <span>
                                    <i v-if="node.data.icon" :class="['iconfont', node.data.icon]"
            ...
            <template #right>
                <div style="text-align: left; height: 100%; overflow: auto;">
                    ...
                    <div
                        style="background: var(--td-bg-color-container); margin: 10px 0px 10px 10px; padding: 20px 20px; border-radius: var(--td-radius-medium);">
                        <t-form ref="formRef" :data="formData" :rules="rules" style="max-width: 650px;"
                            label-align="left" :label-width="90" @submit="onFinish">
                            <t-form-item :label="t('菜单名')" name="name">
                                <t-input v-model="formData.name" clearable></t-input>
                            </t-form-item>

                            <t-form-item :label="t('上级菜单')" name="parentId">
                                <t-tree-select v-if="refreshTree" v-model="formData.parentId" :data="selectTreeData"
                            ...
                            <t-form-item :label="t('类型')" name="type">
                                <t-radio-group v-model="formData.type">
                                    <t-radio v-for="item in menuTypes" :key="item.value" :value="item.value">{{
                                        t(item.label) }}</t-radio>
                                </t-radio-group>
                            </t-form-item>

                            <t-form-item :label="t('图标')" name="icon">
                                <t-select v-model="formData.icon" clearable>
                                    <t-option v-for="item in iconOptions" :key="item.value" :value="item.value"
                                        :label="item.label">
                            ...
                            <t-form-item :label="t('状态')" name="status">
                                <t-radio-group v-model="formData.status">
                                    <t-radio v-for="item in menuStatus" :key="item.value" :value="item.value">{{
                                        t(item.label) }}</t-radio>
                                </t-radio-group>
                            </t-form-item>
