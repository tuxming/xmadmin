<template>
    <Modal :open="visible" :title="t('分配权限')" :width="900" :height="800" @close="onCloseModal">
        <div style="padding: 0px 20px 10px 20px;">
            <h4 style="margin-top: 20px; margin-bottom: 20px; text-align: center;">{{ t('分配权限') }}</h4>

            <div style="height: 500px;">
                <t-transfer v-model="targetKeys" :data="permissions" :keys="{ label: 'name', value: 'key' }"
                    :search="true" :title="[t('未分配'), t('已分配')]">
                    <template #title="{ type }">
