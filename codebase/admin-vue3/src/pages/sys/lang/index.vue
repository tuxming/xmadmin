<template>
    <div style="height: 100%">
        <DoubleColumnLayout :width="containerWidth" :left-width="leftWidth">
            <template #left>
                <div :style="{ height: pos.height ? (pos.height - 90) + 'px' : '100%' }">
                    <t-space>
                        <AuthButton theme="primary" variant="outline" shape="circle" :tip="t('新增语言')"
                            :required-permissions="permission.lang?.langAdd?.expression" @click="onCreateLang">
                            <template #icon><t-icon name="add" /></template>
                        </AuthButton>
                        <AuthButton theme="default" variant="outline" shape="circle" :tip="t('编辑语言')"
                            :required-permissions="permission.lang?.langEdit?.expression" @click="onEditLang">
                            <template #icon><t-icon name="edit" /></template>
                        </AuthButton>
                        <AuthButton theme="danger" variant="outline" shape="circle" :tip="t('删除语言')"
                            :required-permissions="permission.lang?.langDelete?.expression" @click="onLangDelete">
                            <template #icon><t-icon name="delete" /></template>
                        </AuthButton>
                    </t-space>
                    <t-divider size="8px" />

                    <t-menu :value="activeMenuKey" :expanded="openKeys"
            ...
            <template #right>
                <div style="padding-left: 15px; height: 100%; display: flex; flex-direction: column;">
                    ...
                    <t-space wrap>
                        <AuthButton theme="primary" :tip="t('新增资源')"
                            :required-permissions="permission.lang?.resAdd?.expression" @click="onResCreate">
                            <template #icon><t-icon name="add" /></template>
                            {{ !onlyIcon ? t('新增') : '' }}
                        </AuthButton>

                        <AuthButton theme="default" :tip="t('编辑资源')"
                            :required-permissions="permission.lang?.resEdit?.expression" @click="onResEdit">
                            <template #icon><t-icon name="edit" /></template>
                            {{ !onlyIcon ? t('编辑') : '' }}
                        </AuthButton>

                        <AuthButton theme="danger" :tip="t('删除资源')"
                            :required-permissions="permission.lang?.resDelete?.expression" @click="onResDelete">
                            <template #icon><t-icon name="delete" /></template>
                            {{ !onlyIcon ? t('删除') : '' }}
                        </AuthButton>
                    </t-space>
                    ...
                    <div class="table-container">
                        <ResourceList :query="query" :refresh="refresh"
        ...
        <LangEdit v-if="isLangEditOpen" v-model:open="isLangEditOpen" :lang="editLangObj" :title="title"
            @close="onLangEditClose" />

        <ResourceEdit v-if="isResEditOpen" v-model:open="isResEditOpen" :langs="langs" :groups="groups"
            :resource="selectedResource" @close="onResClose" />
    </div>
</template>
